...
export const UserAboutTab = ({ userProfile }: { userProfile?:UserModel }) => {
  if(!userProfile) return <div>loading about</div>;
  const {id,bio}= userProfile;
  return (
    <div className="flex flex-1 flex-col gap-4 sm:gap-8">
      ...
      <p className="gap-2 text-base opacity-75 sm:gap-4 sm:text-lg">
        <span>Joined since {format(userProfile.dateCreated,"MMMM yyyy")}</span>
